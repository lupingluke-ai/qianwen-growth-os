...
function ReviewMaterialPanel({ levelDef, evidences }: { levelDef?: LevelDefinition; evidences: Evidence[] }) {
  ...
  return <div className="review-material">
    <div className="review-material-head"><b>L{levelDef.level} 通关标准与证据</b><em className={coveredCount === levelDef.criteria.length ? "all-covered" : ""}>{coveredCount}/{levelDef.criteria.length} 项已覆盖</em></div>
    ...
    {extras.length ? <div className="review-criterion covered"><div className="review-criterion-head"><Check size={16} /><b>其他证据</b><small>{extras.length} 条</small></div>{extras.map(item => <div className="review-evidence-item" key={item.id}><div><b>{item.title}</b><small>{item.kind} · {item.status} · {formatDate(item.createdAt)}</small>{item.outcome ? <p>{item.outcome}</p> : null}</div>{item.url ? <a href={item.url} target="_blank" rel="noreferrer" aria-label={`打开 ${item.title} 材料`}><ExternalLink size={16} /></a> : null}</div>)}</div> : null}
  </div>;
}
...
export default function Dashboard({ levels: fallbackLevels, industryAnchors, stageMeta }: Props) {
  ...
  const applyWorkspace = useCallback((data: WorkspacePayload) => {
    setWorkspace(data);
    if (data.myMember) {
      const nextLevel = Math.min((data.myMember.currentLevel ?? 0) + 1, data.myMember.targetLevel || 6);
  ...
  const checkinTargetLocked = Boolean(checkinDraft && workspace?.members.find(item => item.id === checkinDraft.memberId)?.pendingReviewId);
  ...
  const nextLevelNumber = Math.min(currentLevel + 1, myMember?.targetLevel || 6);
  ...
  }).toSorted((a, b) => Number(a.checkedInThisMonth) - Number(b.checkedInThisMonth) || b.overdueTasks - a.overdueTasks || b.currentLevel - a.currentLevel), [teamGroup, teamQuery, workspace?.members]);
  ...
  return <main className="control-shell">
    ...
    <section className="control-workspace">
      ...
      {activeView === "growth" && workspace ? <section className="workspace-view growth-view">
        <div className="page-heading compact-heading">
          <div><h1>{workspace.authenticated ? `${workspace.me?.displayName}，专注下一次升级。` : "让每一次成长，都有清晰的下一步。"}</h1><p className="heading-summary">当前 L{myMember?.currentLevel ?? workspace.metrics.median} · 目标 L{myMember?.targetLevel || nextLevelNumber} · {myMember?.progressStatus || "从真实工作结果开始"}</p></div>
        </div>
        ...
        <div className="growth-focus-grid">
          <section className="growth-hero-panel">
            ...
            <div className="growth-level-lockup"><strong>L{myMember?.currentLevel ?? workspace.metrics.median}</strong><div><small>下一级</small><b>L{nextLevelNumber} · {nextLevelDef.title} · {nextLevelEvidence.filter(e => nextLevelDef.criteria.some(c => c.id === e.criterionKey)).length}/{nextLevelDef.criteria.length}已覆盖</b>{nextLevelNumber !== targetLevel.level ? <span>远期目标 L{targetLevel.level} · {targetLevel.title}</span> : null}<span>{myMember ? `计划 ${formatDate(myMember.targetDate)} 前完成` : "从真实工作结果开始"}</span></div></div>
            <div className="growth-progress" aria-label="成长进度">
              {activeLevels.map(level => <button key={level.level} className={`${(myMember?.currentLevel ?? workspace.metrics.median) >= level.level ? "reached" : ""} ${myMember?.targetLevel === level.level ? "target" : ""} ${level.level === nextLevelNumber ? "next" : ""}`} onClick={() => { setFocusedLevelNumber(level.level); setActiveView("capability"); }} aria-label={`查看 L${level.level} ${level.title}`}><i /><span>L{level.level}</span></button>)}
            </div>
            <div className="growth-hero-footer"><span><small>证据</small><b>{myMember ? `${nextLevelEvidence.length} / ${nextLevelDef.criteria.length}` : `${workspace.metrics.evidenceCompletion}%`}</b></span><span><small>评审状态</small><b>{myMember?.reviewStatus || `${workspace.metrics.pendingReviews} 项进行中`}</b></span>{myMember?.gap ? <span><small>差距摘要</small><b>{myMember.gap.substring(0, 30)}{myMember.gap.length > 30 ? "…" : ""}</b></span> : null}{myMember?.updatedAt ? <span><small>最后更新</small><b>{formatDate(myMember.updatedAt)}</b></span> : null}<button className="text-link" onClick={() => { setFocusedLevelNumber(nextLevelNumber); setActiveView("capability"); }}>查看目标标准 <ChevronRight size={16} /></button></div>
          </section>
          ...
          <aside className="next-action-card">
            ...
            {workspace.authenticated && myMember ? <button className="secondary-action full" onClick={() => openCheckin(myMember)}>更新成长进展</button> : null}
          </aside>
        </div>
        ...
        <div className="growth-detail-grid">
          <section className="target-criteria-card">
            ...
            {(() => { const coveredCriteria = nextLevelDef.criteria.filter(c => nextLevelEvidence.some(e => e.criterionKey === c.id)); const total = nextLevelDef.criteria.length; const covered = coveredCriteria.length; if (covered >= total && total > 0) { if (myMember?.pendingReviewId) { return <div className="checklist-ready">⏳ 晋级申请评审中</div>; } return <button type="button" className="checklist-ready" onClick={() => { setReviewerEmail(""); setDialogStep(1); setSubmitSuccess(false); setFormErrors({}); setReviewSubmitOpen(true); }}>✅ 可提交晋级申请 →</button>; } return <div className="checklist-pending">还需 {total - covered} 条证据覆盖通关标准</div>; })()}
          </section>
      ...
      {activeView === "team" && workspace ? <section className="workspace-view team-view">
        ...
        <div className="section-tabs"><button className={teamTab === "members" ? "active" : ""} onClick={() => setTeamTab("members")}><Users size={20} />成员概览</button><button className={teamTab === "assets" ? "active" : ""} onClick={() => setTeamTab("assets")}><Library size={20} />成果库</button>{workspace.monthlyReport ? <button className={teamTab === "report" ? "active" : ""} onClick={() => setTeamTab("report")}><Gauge size={20} />月度报告</button> : null}</div>
        {teamTab === "members" ? <>
          <div className="team-stats-overview">
            <div className="stat-card stat-card-wide">
              <h4>层级分布</h4>
              <div className="level-distribution" role="img" aria-label={`团队层级分布：${workspace.metrics.distribution.map((count, index) => `L${index + 1} ${count}人`).join('，')}`}>
                {workspace.metrics.distribution.map((count, index) => count > 0 ? (
                  <div key={index} className="level-bar">
                    <span className="level-count">{count}</span>
                    <div className="level-bar-fill" style={{ height: `${Math.max(count * 18, 6)}px` }} />
                    <span className="level-label">L{index + 1}</span>
                  </div>
                ) : null)}
              </div>
            </div>
            <div className="stat-card"><div className="stat-number">{workspace.metrics.memberCount}</div><div className="stat-label">总人数</div></div>
            <div className="stat-card"><div className="stat-number">{workspace.members.filter(m => { const d = m.updatedAt ? new Date(m.updatedAt).getTime() : 0; return Date.now() - d < 7 * 24 * 60 * 60 * 1000; }).length}</div><div className="stat-label">本周活跃</div></div>
            <div className="stat-card"><div className="stat-number">{workspace.reviews.filter(r => ["已提交", "评审中", "待补证"].includes(r.state)).length}</div><div className="stat-label">进行中评审</div></div>
          </div>
          ...
          {workspace.me?.role === "admin" && selectedAssets.length > 0 && <div className="batch-actions"><span>已选 {selectedAssets.length} 项</span><button className="primary-action" disabled={busy} onClick={async () => { for (const id of selectedAssets) { await mutate({ action: "review_asset", assetId: Number(id), decision: "已发布" }, ""); } setSelectedAssets([]); showToast(`已批量发布 ${selectedAssets.length} 项成果`); }}>全部发布</button><button className="secondary-action" disabled={busy} onClick={async () => { for (const id of selectedAssets) { await mutate({ action: "review_asset", assetId: Number(id), decision: "待补充" }, ""); } setSelectedAssets([]); showToast(`已批量退回 ${selectedAssets.length} 项成果`); }}>全部退回</button></div>}
          <div className="assets-grid"><section className="asset-library-panel"><div className="asset-list">{filteredAssets.map(asset => <div key={asset.id} className="asset-row-wrapper">{workspace.me?.role === "admin" && asset.reviewStatus === "待审核" && <input type="checkbox" checked={selectedAssets.includes(String(asset.id))} onChange={e => { if (e.target.checked) { setSelectedAssets(prev => [...prev, String(asset.id)]); } else { setSelectedAssets(prev => prev.filter(id => id !== String(asset.id))); } }} />}<AssetRow asset={asset} isAdmin={workspace.me?.role === "admin"} myMemberId={workspace.me?.memberId ?? null} onReview={decision => mutate({ action: "review_asset", assetId: asset.id, decision }, `成果已更新为“${decision}”`)} onResubmit={() => mutate({ action: "resubmit_asset", assetId: asset.id }, "成果已重新提交审核")} busy={busy} /></div>)}{!filteredAssets.length ? <EmptyState icon={<Library size={20} />} title="没有符合条件的成果" copy="调整搜索或类型筛选后重试。" /> : null}</div></section><aside className="anchor-panel"><div className="panel-heading"><div><h2>行业实战锚点</h2><p>成果从真实业务任务中沉淀</p></div><BookOpen size={20} /></div>{industryAnchors.map(anchor => <button key={anchor.name} onClick={() => setSelectedAnchor(anchor)}><span><b>{anchor.name}</b><small>{anchor.items.length} 项锚点任务</small></span><ChevronRight size={16} /></button>)}</aside></div>
        </> : workspace.monthlyReport ? <>
          <div className="report-summary">
            <span><b>{workspace.monthlyReport.participationRate}%</b><small>参与率（{workspace.monthlyReport.updatedThisMonth}/{workspace.monthlyReport.memberCount}）</small></span>
          ...
          <div className="report-grid">
            <section className="report-panel">
              <div className="panel-heading"><div><h2>层级分布</h2><p>当前认证层级人数 · 平均 L{workspace.metrics.average} · 中位 L{workspace.metrics.median}</p></div></div>
              <div className="distribution-chart" role="img" aria-label={`团队层级分布：${workspace.metrics.distribution.map((count, index) => `L${index + 1} ${count}人`).join('，')}`}>{workspace.metrics.distribution.map((count, index) => { const maxCount = Math.max(...workspace.metrics.distribution, 1); return <div key={index} className="distribution-col"><b>{count || ""}</b><i style={{ height: `${Math.max(count / maxCount * 100, count ? 8 : 2)}%` }} /><span>L{index + 1}</span></div>; })}</div>
            </section>
            <section className="report-panel">
              <div className="panel-heading"><div><h2>本月晋级名单</h2><p>{workspace.monthlyReport.cycle} 周期通过评审的晋级</p></div><BadgeCheck size={20} /></div>
              {workspace.monthlyReport.promotions.length ? <div className="promotion-list">{workspace.monthlyReport.promotions.map(item => <div className="promotion-row" key={item.id}><span className="member-avatar">{initials(item.memberName)}</span><span><b>{item.memberName}</b><small>{formatDate(item.createdAt)}</small></span><em>L{item.fromLevel} → L{item.toLevel}</em></div>)}</div> : <EmptyState icon={<BadgeCheck size={20} />} title="本月暂无晋级" copy="通过评审的晋级会出现在这里。" />}
            </section>
          </div>
        </> : null}
      </section> : null}
    ...
    {onboardingDraft ? <DialogFrame title="欢迎加入千问计划" onClose={() => setOnboardingDraft(null)} size="wide"><div className="dialog-form">
      <div className="dialog-heading"><span>WELCOME · STEP {onboardingDraft.step}/3</span><h2>{onboardingDraft.step === 1 ? "先告诉我们你在哪里" : onboardingDraft.step === 2 ? "设定你的成长目标" : "写下你的下一步行动"}</h2></div>
      <div className="onboarding-steps" aria-hidden="true">{[1, 2, 3].map(step => <span key={step} className={onboardingDraft.step >= step ? "active" : ""} />)}</div>
      ...
      {onboardingDraft.step === 2 ? <div className="form-grid"><label>目标层级<select value={onboardingDraft.targetLevel} onChange={event => setOnboardingDraft({ ...onboardingDraft, targetLevel: Number(event.target.value) })}>{activeLevels.filter(level => level.level >= (myMember?.currentLevel ?? 0)).map(level => <option key={level.level} value={level.level}>L{level.level} · {level.title}</option>)}</select></label></div> : null}
      {onboardingDraft.step === 3 ? <><label>下一步行动<input value={onboardingDraft.nextTask} onChange={event => setOnboardingDraft({ ...onboardingDraft, nextTask: event.target.value })} placeholder="例如：用 AI 完成一次客户拜访前的情报汇总" /></label><label>目标日期<input type="date" value={onboardingDraft.targetDate} onChange={event => setOnboardingDraft({ ...onboardingDraft, targetDate: event.target.value })} /></label></> : null}
      <div className="form-actions">
        ...
        {onboardingDraft.step < 3 ? <button type="button" className="primary-action" onClick={() => setOnboardingDraft({ ...onboardingDraft, step: onboardingDraft.step + 1 })}>下一步 <ArrowRight size={16} /></button> : <button type="button" className="primary-action" disabled={busy || !onboardingDraft.nextTask.trim()} onClick={async () => { const ok = await mutate({ action: "complete_onboarding", groupName: onboardingDraft.groupName, industry: onboardingDraft.industry, targetLevel: onboardingDraft.targetLevel, targetDate: onboardingDraft.targetDate, nextTask: onboardingDraft.nextTask }, "欢迎加入千问计划，成长档案已建立"); if (ok) { setOnboardingDraft(null); openEvidence(workspace?.myMember, nextLevelNumber); } }}>{busy ? "保存中…" : "完成引导"}</button>}
      </div>
    </div></DialogFrame> : null}

    {checkinDraft ? <DialogFrame title="更新成长进展" onClose={() => setCheckinDraft(null)} size="wide"><form className="dialog-form" onSubmit={async (event: FormEvent) => { event.preventDefault(); const ok = await mutate({ action: "update_checkin", ...checkinDraft }, "进展已更新"); if (ok) setCheckinDraft(null); }}>
      <div className="dialog-heading"><span>PERSONAL CHECK-IN</span><h2>更新成长进展</h2></div>
      <div className="certified-banner"><BadgeCheck size={20} /><span><small>当前认证层级</small><b>L{workspace?.members.find(item => item.id === checkinDraft.memberId)?.currentLevel ?? 0} · 不可直接修改</b></span></div>
      <div className="form-section-label collapsible" onClick={() => setGoalExpanded(prev => !prev)} role="button" tabIndex={0}>我的目标：L{myMember?.currentLevel ?? 0} → L{myMember?.targetLevel || 6}<small>{goalExpanded ? "收起" : "展开调整"}</small></div>
      {goalExpanded ? <>
        <div className="form-grid"><label>目标层级<select value={checkinDraft.targetLevel} disabled={checkinTargetLocked} onChange={event => setCheckinDraft({ ...checkinDraft, targetLevel: Number(event.target.value) })}>{activeLevels.filter(level => level.level >= (workspace?.members.find(item => item.id === checkinDraft.memberId)?.currentLevel ?? 0)).map(level => <option key={level.level} value={level.level}>L{level.level} · {level.title}</option>)}</select><span className="field-hint">可设远期目标，系统引导逐级通关</span></label><label>目标日期<input type="date" value={checkinDraft.targetDate} onChange={event => setCheckinDraft({ ...checkinDraft, targetDate: event.target.value })} /></label></div>
        {checkinTargetLocked ? <div className="readonly-notice"><LockKeyhole size={20} /><span>有进行中的晋级申请，撤回后才能调整目标层级。</span></div> : null}
      </> : null}
      <div className="form-section-label">本次进展<small>建议每周更新一次</small></div>
      <div className="form-grid"><label>推进状态<select value={checkinDraft.progressStatus} onChange={event => setCheckinDraft({ ...checkinDraft, progressStatus: event.target.value })}>{["正常", "进行中", "有风险", "阻塞"].map(status => <option key={status}>{status}</option>)}</select></label><label>下一步任务<input value={checkinDraft.nextTask} onChange={event => setCheckinDraft({ ...checkinDraft, nextTask: event.target.value })} placeholder="当前最重要的一件事" /></label></div>
      <label>当前差距<textarea rows={3} value={checkinDraft.gap} onChange={event => setCheckinDraft({ ...checkinDraft, gap: event.target.value })} placeholder="对照目标层级，描述还缺少什么" /></label>
...
function NewUserForm({ busy, onCreate }: { busy: boolean; onCreate: (fields: { email: string; displayName: string; password: string; role: string; groupName: string }) => Promise<boolean> }) {
  ...
  return <form className="new-user-form" onSubmit={async event => { event.preventDefault(); const ok = await onCreate({ email, displayName, password, role, groupName }); if (ok) { setDisplayName(""); setEmail(""); setPassword(""); setRole("member"); setGroupName("综合组"); } }}>
    ...
    <div className="new-user-grid">
      <label><small>姓名</small><input required value={displayName} onChange={event => setDisplayName(event.target.value)} placeholder="真实姓名" /></label>
      <label><small>邮箱</small><input required type="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="name@qianwen" /></label>
      <label><small>初始密码</small><input required type="password" minLength={6} value={password} onChange={event => setPassword(event.target.value)} placeholder="至少 6 位" /></label>
    ...
    <span className="new-user-hint">新成员首次登录会自动进入 3 步引导完成建档</span>
    <button className="primary-action" disabled={busy || !emailValid || !passwordValid}>{busy ? "创建中…" : "创建账号"}</button>
  </form>;
}

function AssetRow({ asset, isAdmin, myMemberId, onReview, onResubmit, busy }: { asset: AssetRecord; isAdmin: boolean; myMemberId: number | null; onReview: (decision: string) => Promise<boolean>; onResubmit: () => Promise<boolean>; busy: boolean }) {
  return <article className="asset-row"><div className="asset-icon">{asset.type === "Skill" ? <Sparkles size={20} /> : asset.type === "知识库" ? <BookOpen size={20} /> : asset.type === "评测集" ? <PackageCheck size={20} /> : <FolderKanban size={20} />}</div><div className="asset-copy"><div><h3>{asset.title}</h3><span>{asset.type} · {asset.industry}</span></div><p>{asset.ownerName} · 更新于 {formatDate(asset.updatedAt)}{asset.sourceEvidenceId ? " · 来自成长证据" : ""}</p></div><div className="asset-reuse"><b>{asset.reusePeople}</b><small>人复用</small></div><div className="asset-states"><em className={`state-label ${toneClass(asset.reviewStatus)}`} aria-label={`审核状态：${asset.reviewStatus}`}>{asset.reviewStatus}</em><small><ShieldCheck size={16} />{asset.complianceStatus}</small></div>{isAdmin && asset.reviewStatus === "待审核" ? <div className="asset-review-actions"><button disabled={busy} onClick={() => onReview("待补充")}>退回</button><button disabled={busy} onClick={() => onReview("已发布")}>发布</button></div> : asset.reviewStatus === "待补充" && (isAdmin || asset.ownerMemberId === myMemberId) ? <div className="asset-review-actions"><button disabled={busy} onClick={onResubmit}>重新提交</button></div> : asset.url ? <a href={asset.url} target="_blank" rel="noreferrer" aria-label={`打开 ${asset.title}`}><ExternalLink size={16} /></a> : <span className="muted-link">—</span>}</article>;
}
